...
from (select c_custkey, count(o_orderkey) as c_count from customer left outer join orders on c_custkey = o_custkey and o_comment not like '%pending%deposits%' group by c_custkey) c_orders group by c_count
...
from partsupp, part
where p_partkey = ps_partkey and p_brand <> 'Brand#34' and p_type not like 'LARGE BRUSHED%'	and p_size in (48, 19, 12, 4, 41, 7, 21, 39) and ps_suppkey not in ( select s_suppkey from supplier where s_comment like '%Customer%Complaints%')
group by p_brand,
	p_type,
	p_size
order by
...
from lineitem, part
where p_partkey = l_partkey and p_brand = 'Brand#44' and p_container = 'WRAP PKG' and l_quantity < ( select 0.2 * avg(l_quantity) from lineitem where l_partkey = p_partkey);
"""

q18 = """
select c_name,c_custkey,o_orderkey,	o_orderdate, o_totalprice, sum(l_quantity)
from customer, orders,lineitem
where o_orderkey in (select l_orderkey from lineitem group by l_orderkey having sum(l_quantity) > 314) and c_custkey = o_custkey and o_orderkey = l_orderkey
...
where s_suppkey = l1.l_suppkey and o_orderkey = l1.l_orderkey
	...
	and exists ( select * from lineitem l2 where l2.l_orderkey = l1.l_orderkey and l2.l_suppkey <> l1.l_suppkey) and not exists ( select * from lineitem l3 where l3.l_orderkey = l1.l_orderkey and l3.l_suppkey <> l1.l_suppkey and l3.l_receiptdate > l3.l_commitdate) and s_nationkey = n_nationkey and n_name = 'EGYPT'
group by
	s_name
order by
	numwait desc,
...
class Sql:
    ...
    def addInfo(self, executed_time):
        ...
        if executed_time < self.min_time:
            self.min_time = executed_time
        self.total_count += 1
        lock.release()
    ...
    def getInfo(self):
        info = "%s[min time: %f, avg time: %fs, total count:%d]" % (
            self.sql_name, self.min_time, self.getAvgTime(), self.getCount())
        return info
    ...
    with connection:
        with connection.cursor() as cursor:
            for t in range(1, test_time + 1):
                if isFirst == False:
                    cursor.execute("set tidb_enforce_mpp=true;")
                    cursor.execute("set @@session.tidb_allow_mpp=true;")
...
def parse_args():
    ...
    parser.add_argument('--t_time', type=int, default=1,
                        help="test time), default 1")
