...
struct WaitingNodeFiber : public SimpleIntrusiveNode<WaitingNodeFiber>
{
...
/// This can significantly reduce contentions and avoid "thundering herd" problem.
template <typename T>
class MPMCQueueFiber
{
public:
    using Status = MPMCQueueStatus;
    using Result = MPMCQueueResult;

    explicit MPMCQueueFiber(size_t capacity_)
        : capacity(capacity_)
        , data(capacity * sizeof(T))
    {
    }
    ...
    // because MPMCQueue maybe used by different threads.
    // Copy and move it is dangerous.
    DISALLOW_COPY_AND_MOVE(MPMCQueueFiber);
...
private:
    using TimePoint = std::chrono::time_point<std::chrono::system_clock>;
    using WaitingNode = MPMCQueueDetail::WaitingNodeFiber;
    ...
    template <typename Pred>
    ALWAYS_INLINE bool wait(
        std::unique_lock<FiberTraits::Mutex> & lock,
        WaitingNode & head,
        WaitingNode & node,
        Pred pred,
        const TimePoint * deadline)
    {
        if (deadline)
        {
            while (!pred())
            {
                ...
                auto res = node.cv.wait_until(lock, *deadline);
                node.detach();
                if (res == boost::fibers::cv_status::timeout)
                    return false;
            }
        }
        else
