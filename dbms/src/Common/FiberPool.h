...
inline void adaptive_yield()
{
    if (g_run_in_fiber)
    {
        // std::cout << "run in fiber.." << std::endl;
        boost::this_fiber::yield();
    }
    else
    {
        // std::cout << "run in thread.." << std::endl;
...
{
    return std::max(std::thread::hardware_concurrency() * 10, 2u) - 1u;
}
...
template <
    ...
    typename WorkTaskT = std::tuple<boost::fibers::launch,
...
public:
    ...
    auto threadsNo() const noexcept
    {
        return m_threads.size();
    }

    auto fibersNo() const noexcept
    {
...
template <
    template <typename> typename TaskQueueT
...
template <typename Func, typename... Args>
inline auto
submitJob(boost::fibers::launch launch_policy,
          Func && func,
          Args &&... args)
{
