...
UInt64 DecodeVarUInt(size_t & cursor, const String & raw_value)
{
    ...
    {
        UInt64 v = raw_value[cursor++]; // NOLINT(bugprone-signed-char-misuse)
        if (v < 0x80)
        {
            if (i > 9 || (i == 9 && v > 1))
...
{
    PrecType prec = raw_value[cursor++]; // NOLINT(bugprone-signed-char-misuse)
    ScaleType scale = raw_value[cursor++]; // NOLINT(bugprone-signed-char-misuse)
...
{
    PrecType prec = raw_value[cursor++]; // NOLINT(bugprone-signed-char-misuse)
    ScaleType frac = raw_value[cursor++]; // NOLINT(bugprone-signed-char-misuse)
...
void EncodeDecimalImpl(const T & dec, PrecType prec, ScaleType frac, WriteBuffer & ss)
{
    ...
    int words_frac = frac / digitsPerWord;
    int trailing_digits = frac - words_frac * digitsPerWord;
    int words = getWords(prec, frac); // NOLINT(bugprone-signed-char-misuse))
